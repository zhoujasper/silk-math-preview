...
## 文档与发布语言（长期约定，2026-09-09）

- `CHANGELOG.md` 的每个版本都必须有完整的中英双语内容，固定先 `### 中文`、后 `### English`；更新项、修复、限制和已知问题逐项对应，不得只翻译标题或给另一种语言写摘要。
- 后续新版本的发布说明（包括 GitHub Release）同样必须使用中英双语，优先直接取对应版本的 CHANGELOG；发布前核对版本号、两种语言的含义和条目完整性。CI 使用 `scripts/release-notes.mjs` 检查并提取，不能退回单语自动提交摘要。
- `README.md` 保留当前 11 种语言：英文、简体中文、繁體中文、日本語、한국어、Deutsch、Français、Español、Português、Русский、Italiano。后续介绍更新继续维护这些语言，保持单文件与页内导航，不缩减为中英两种。
- README 的各语言按同一功能清单完整更新：支持范围、宏与依赖、安装要求、操作与快捷键、默认开关、限制及文档入口均须对应；新增功能不可只写中英，也不要把同一功能追加成重复条目。核对语义及实际行为，不能只比较字数或条目数。
- 本地化使用当地惯用术语和自然句式，繁中沿用台湾用语、葡语沿用巴西葡语；产品名、TeX 命令、配置键与版本号保持准确。菜单名称对照实际界面，尚为英文的 OCR 入口保留英文名称并附当地语言解释；详细文档未翻译时标注其实际语言。含反斜杠的公式示例使用 Markdown 行内代码，避免内联 HTML 中的转义损失。
...
## 2026-09-09 README 多语言补全与本地化

- 主代理独立完成 11 种语言的逐项补全与改写；原有繁中、日、韩及欧洲语言缺少的宏包、CSS、TikZ、OCR 操作/快捷键/测试版说明已补齐，英文也同步中文原有细节。保留单文件、语言顺序、页内导航、4 张图片和许可页脚。
- 同期其他任务新增的 OCR 留白/背景/重试/表格复核说明与 CSS 高级定位、间距、字号、防覆盖、补全/悬停/示例均保留并本地化。合并了并发新增的重复 CSS 条目；本轮只编辑 README、本文件和项目记忆，不修改其功能实现。
- Markdown 解析核验通过：每种语言 4 段介绍、8 个功能条目、31 个一致的代码示例/标识（德语 Strg 对应 Ctrl）、4 个相同目标的文档链接，共 88 条功能说明与 44 个文档引用。菜单与当前语言目录/实际 OCR 回退一致，导航/本地资源有效，数学反斜杠保留，韩语菜单加粗修复。
- 现有 README/语言目录 2 个测试文件共 14 项通过，文档差异空白检查通过。沿用已有构建执行 benchmark/size：cold p50/p95 47.95/53.50 ms、warm 3.83/5.33 ms、scanner p95 0.475 ms，idle restart 通过；主 bundle 201,830 B，既有 VSIX 均在硬门内。这不是 README 带来的性能提升或新安装包验证。
- 本轮未打开浏览器/Extension Host，未重新构建、打包、安装、提交、推送或发布；既有 VSIX 不视为已包含本次 README。保留工作区其他任务的修改。

## 2026-09-09 OCR 背景、留白与结构优化（0.2.5）

- 主代理独立完成。原图在本机旧 0.2.2 包和修改前源码下能读全；增加留白或改变背景后，真实复现空数组、孤立横线、字母误读且错误标成功。新流程先在原分辨率定位全部笔画并统一背景、裁空白，保留右侧条件、标点和抗锯齿，再等比例送入模型。
- 异常结构/低置信度最多重识别一次；正常只一次，空白不加载模型，EOS/花括号/环境/孤立横线/行列及真实格线核验。单行空数组提取只是严格条件下的回退，不补写模型漏字；真实矩阵、分段、对齐及表格保留。智能模式遇到需要复核的表格仍保留整表。
- 相关 9 files / 62 tests、类型检查通过。VS Code Node 24.18.1 真实 Worker 19 次检查：16 次公式/矩阵/分段/背景与留白识别通过，3 次真实表格结果保留并明确复核。表格仍有模型多列/边线错误，不能宣称已准确识别全部表格。原图 cold 2.070 秒，5 个背景/留白变体 warm 1.407–1.578 秒，仅代表本机。
- 数学输出压缩无意义空白，保留文字参数和控制词边界。沿用原有 94 MiB 模型包，无下载、训练或新依赖；普通预览 Node 26 cold p50/p95 48.07/51.87 ms、warm 3.85/6.36 ms，scanner p95 0.459 ms，idle restart 通过。测试图仅进入测试目录，不打包。
- 版本与双语 CHANGELOG 同步为 0.2.5，README 11 种语言均补充识别行为；详细边界与复测方法见 `docs/OCR_ACCURACY.md`。保留同目录预览布局及发布流程的其他修改；最终 `0.2.5-ocr.vsix` 以已验证 0.2.4 双通道包为底座，只更新 OCR Worker、清单版本与说明，不包含同期未完成的布局修改。除 5 个指定条目外均与对应旧包逐字节相同，GPL Worker 与 15 份对应源码原样保留。
- 最终包内 OCR Worker 228,493 B、主包正式/测试 201,065/200,995 B；两包各 24 条目、ZIP/命名空间/默认开关/双语说明/禁入文件检查通过。正式包内 Worker 在 VS Code Node 24.18.1 再次完成全部 19 项（16 识别通过、3 保留复核），测试包独立复测原图；两包 OCR 字节一致并匹配当前 `src/ocr` 构建。识别原图输出与规范 `\mathbf` 写法生成的 SVG 逐字节一致。未安装、提交、推送或发布，未打开浏览器/Extension Host。

## 2026-09-09 其他文件与短结束行预览修复（0.2.4）

- 主代理独立处理截图反馈。真实状态栏策略原本允许 `untitled`/纯文本进入 LaTeX 扫描；重建截图可见数组后，独立 Worker 能生成 SVG。定位发现 viewport anchor 不可用时 `100%` 回退使用 Monaco 短结束行的文字容器，可能把正常图片缩到几像素；宽度和右/上下边界回退现改为已知图片尺寸及正常落点，保留真实 anchor 限位。未改文件类型默认值或源码内容。
- 增加 5 项回归，覆盖保存/未保存纯文本、开启后立即显示/关闭后清除，以及上/下方短行尺寸。直接加载旧 0.2.3 和两份新 VSIX 的布局模块：无有效 anchor、16px 结束行、126×88px 图片时，旧宽度 8px，新两通道均 126×88px，落点恢复 22px；这是 CSS 回退计算，不是图形界面截图验收。
- `npm run verify` 36 files / 414 tests、typecheck、coverage、build、benchmark、size 全通过；核心 stmts/branch/lines 93.88%/88.46%/96.52%。Node 26.8.1 本机 cold p50/p95 47.97/52.87 ms、warm 4.02/7.99 ms、scanner p95 0.499 ms，idle restart 通过。主 bundle 201,065 B，布局模块 9,251 B。
- 两份实际 VSIX 的独立 Worker 在 VS Code Node 24.18.1 下各完成数组+光标与后续公式请求；数组 125.72×88.08px、渲染约 9.1ms。ZIP/通道/默认开关/当前双语说明/Worker 与构建/15 份 GPL 对应源码逐文件一致通过。每包 24 条目，dist 恢复正式通道。
...
- 保留同期菜单失焦关闭、CSS、历史双语日志与 CI 改动及旧 VSIX；未安装、提交、推送或发布。遵照用户要求未打开浏览器或 Extension Host；完整原公式和实机视觉由用户验收。无有效 anchor 时窄分屏的精确边界仍有限制，见 `docs/PREVIEW_CSS.md`。

## 2026-09-09 状态栏菜单点击外部关闭（0.2.3）

- 主代理独立修复：状态栏 QuickPick 的 `ignoreFocusOut` 改为 false，点击外部移走焦点时走已有隐藏/释放流程；菜单内缩放、开关仍保持打开并即时刷新。正式/测试通道共用此逻辑。
- 轻量验证：类型检查、2 个相关测试文件共 17 项、双语发布说明提取、双通道构建/打包及体积检查通过；两份实际 VSIX 均确认失焦关闭设置、通道隔离、版本与 ZIP 完整性，当前双语说明一致。主 bundle 201,065 B。
- 本机普通公式 cold p50/p95 47.26/52.37 ms、warm 3.65/5.25 ms、scanner p95 0.437 ms，idle restart 通过。未运行全量覆盖率、浏览器或 Extension Host；实际点击由用户验收。两个 0.2.3 VSIX 已生成，dist 恢复正式通道；旧包与其他任务修改保留，未安装、提交、推送或发布。

## 2026-09-09 更新日志与发布双语

- 主代理独立补齐 CHANGELOG 的全部 85 个版本（0.1.0 至 0.2.2）；原有版本标题、日期与正文保留，新增另一种语言。工作区同期新增的 0.2.2 记录一并翻译。
- 自动发布改为提取当前版本的双语说明，用于 GitHub Release 创建及更新；本轮未改写 README，保留同期 CSS 功能任务新增的介绍和全部 11 种语言。只检查文档与发布说明流程，本轮不构建、打包或发布扩展。
- 核验：85 个版本、185 对条目完整对应；版本标题、日期及原语言正文完整保留。提取脚本的历史/当前版本、CRLF、准确版本匹配与缺失/重复/单语/空条目错误分支通过；CI YAML、shell 语法及创建/更新分支使用本地 gh 替身验证通过，未访问远端发布。
...
- 除上述文档与发布语言约定外，面向用户的说明、诊断和注释优先使用中文；API 标识、TeX 命令和协议字段保留英文。
